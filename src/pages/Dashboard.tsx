...
import { 
  ...
  Trophy, TrendingDown, Heart, Apple, Pill
} from 'lucide-react';
  ...
  return (
    <div className="space-y-5">
      ...
      {activeNutrition && todayNutritionDay && (
        <div className={`rounded-2xl p-5 border theme-transition ${
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className={`text-xs ${isDark ? 'text-gray-400' : 'text-[#0f766e]/70'}`}>
                  {todayNutritionDay.day} • {activeNutrition.plan_name}
                </p>
              </div>
            </div>
            <button
              onClick={() => navigate('/nutrition')}
              className={`text-xs font-bold ${isDark ? 'text-[#22c55e] hover:underline' : 'text-[#059669] hover:underline'}`}
            >
              جزئیات ←
            </button>
          </div>
              ...
              <div key={i} className={`flex items-center justify-between rounded-lg px-3 py-2 ${
                ...
                <div>
                  ...
                  <p className={`text-[11px] ${isDark ? 'text-gray-500' : 'text-[#0f766e]/50'}`}>
                    {(meal.foods || []).map((f: any) => f.name).join('، ') || meal.time}
                  </p>
                </div>
                {meal.time && (
          ...
          <div className={`mt-3 flex items-center justify-between text-xs ${
            isDark ? 'text-gray-400' : 'text-[#0f766e]/70'
          }`}>
      ...
      {activeSupplement && activeSupplement.supplements && activeSupplement.supplements.length > 0 && (
        <div className={`rounded-2xl p-5 border theme-transition ${
          isDark 
            ? 'bg-gradient-to-l from-[#4a90d9]/5 to-transparent border-[#4a90d9]/20' 
            : 'bg-gradient-to-l from-[#eff6ff] to-white border-[#3b82f6]/20'
        }`}>
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-3">
              <div className={`w-11 h-11 rounded-xl flex items-center justify-center ${
                isDark ? 'bg-[#4a90d9]/20' : 'bg-[#3b82f6]/15'
              }`}>
                <Pill size={20} className={isDark ? 'text-[#4a90d9]' : 'text-[#2563eb]'} />
              </div>
              <div>
                <h3 className={`font-bold ${isDark ? 'text-[#4a90d9]' : 'text-[#2563eb]'}`}>
                  مکمل‌های امروز
                </h3>
                <p className={`text-xs ${isDark ? 'text-gray-400' : 'text-[#0f766e]/70'}`}>
                  {activeSupplement.recommendation_title}
                </p>
              </div>
            </div>
            <button
              onClick={() => navigate('/supplements')}
              className={`text-xs font-bold ${isDark ? 'text-[#4a90d9] hover:underline' : 'text-[#2563eb] hover:underline'}`}
            >
              جزئیات ←
            </button>
          </div>
