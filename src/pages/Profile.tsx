...
export default function Profile() {
  ...
  const handleSave = () => {
    const profile: AthleteProfile = {
      ...
      name: form.name || 'ورزشکار', age: form.age || 25, gender: form.gender || 'male',
      height: form.height || 175, weight: form.weight || 75, targetWeight: form.targetWeight,
      activityLevel: form.activityLevel || 'moderate', experience: form.experience || 'intermediate',
      trainingDays: form.trainingDays || 4, sessionDuration: form.sessionDuration || 60,
      location: form.location || 'gym', equipmentType: form.equipmentType || 'full_gym',
      customEquipment: form.customEquipment || [], equipment: form.equipment || [],
      injuries: form.injuries || [], limitations: form.limitations || [],
      avoidedExercises: form.avoidedExercises || [], primaryGoal: form.primaryGoal || 'hypertrophy',
      secondaryGoal: form.secondaryGoal, targetMuscles: form.targetMuscles || [],
      programType: form.programType || 'full_body', timeline: form.timeline || '',
      trainingHistory: form.trainingHistory || '', strengthRecords: form.strengthRecords || {},
      bodyMeasurements: form.bodyMeasurements || {}, dietaryGoal: form.dietaryGoal || '',
      dietType: form.dietType || '', foodAllergies: form.foodAllergies || [],
      favoriteFoods: form.favoriteFoods || [], dislikedFoods: form.dislikedFoods || [],
      mealsPerDay: form.mealsPerDay || 3, calorieTarget: form.calorieTarget,
      cookingSkill: form.cookingSkill || 'basic', supplementGoal: form.supplementGoal || '',
      currentSupplements: form.currentSupplements || [], supplementBudget: form.supplementBudget || '',
      ...
      <div className="space-y-6">
        ...
        {profiles.length === 0 ? (
          <div className={'rounded-2xl p-10 border text-center theme-transition ' + (isDark ? 'bg-[#1a1a2e] border-[#d4af37]/10' : 'bg-white border-[#14b8a6]/20')}>
            ...
            <button onClick={handleNewProfile} className={'px-6 py-2.5 rounded-xl font-bold text-sm transition-all ' + (isDark ? 'bg-gradient-to-l from-[#d4af37] to-[#f0d060] text-[#0d0d1a] hover:opacity-90' : 'bg-gradient-to-l from-[#14b8a6] to-[#0d9488] text-white hover:opacity-90')}>
              ایجاد پروفایل جدید
            </button>
          </div>
        ) : (
              ...
              <div key={profile.id} className={'rounded-2xl p-5 border theme-transition ' + (profile.id === activeProfile?.id ? (isDark ? 'bg-gradient-to-l from-[#d4af37]/10 to-transparent border-[#d4af37]/50 shadow-lg shadow-[#d4af37]/10' : 'bg-gradient-to-l from-[#14b8a6]/10 to-white border-[#14b8a6]/50 shadow-lg shadow-[#14b8a6]/10') : (isDark ? 'bg-[#1a1a2e] border-gray-800 hover:border-gray-600' : 'bg-white border-[#14b8a6]/20 hover:border-[#14b8a6]/40'))}>
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-3">
                    <div className={'w-12 h-12 rounded-xl flex items-center justify-center font-bold text-lg ' + (profile.id === activeProfile?.id ? (isDark ? 'bg-gradient-to-br from-[#d4af37] to-[#f0d060] text-[#0d0d1a]' : 'bg-gradient-to-br from-[#14b8a6] to-[#0d9488] text-white') : (isDark ? 'bg-gray-800 text-gray-400' : 'bg-[#f0fdfa] text-[#0d9488]'))}>
                  ...
                  {profile.id === activeProfile?.id && (
                    <span className={'text-xs px-2 py-1 rounded-full font-bold ' + (isDark ? 'bg-[#22c55e]/20 text-[#22c55e]' : 'bg-[#10b981]/15 text-[#059669]')}>فعال</span>
                  )}
                </div>
