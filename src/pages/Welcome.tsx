  ...
  return (
    <main dir="rtl" className={`min-h-screen flex items-center justify-center p-6 ${isDark ? 'bg-[#0d0d1a] text-white' : 'bg-gradient-to-br from-white to-[#f0fdfa] text-[#134e4a]'}`}>
      <section className="w-full max-w-md text-center">
        <div className={`mx-auto mb-7 w-24 h-24 rounded-3xl flex items-center justify-center shadow-2xl ${isDark ? 'bg-gradient-to-br from-[#d4af37] to-[#f0d060] shadow-[#d4af37]/20' : 'bg-gradient-to-br from-[#14b8a6] to-[#0d9488] shadow-[#14b8a6]/20'}`}>
          <Dumbbell size={44} className={isDark ? 'text-[#0d0d1a]' : 'text-white'} />
        </div>
        <div className="flex items-center justify-center gap-2 mb-2">
          <Sparkles size={17} className={isDark ? 'text-[#d4af37]' : 'text-[#0d9488]'} />
          <span className={`text-sm font-bold ${isDark ? 'text-[#d4af37]' : 'text-[#0d9488]'}`}>دستیار هوشمند بدنسازی</span>
        </div>
        ...
        <div className={`mt-8 rounded-3xl p-6 border ${isDark ? 'bg-[#1a1a2e] border-[#d4af37]/20' : 'bg-white border-[#14b8a6]/20 shadow-lg'}`}>
        ...
        <button onClick={onContinue} className={`mt-7 w-full py-4 rounded-2xl font-extrabold flex items-center justify-center gap-2 transition-transform active:scale-[0.98] ${isDark ? 'bg-gradient-to-l from-[#d4af37] to-[#f0d060] text-[#0d0d1a]' : 'bg-gradient-to-l from-[#14b8a6] to-[#0d9488] text-white'}`}>
          ورود به کوچینو
          <ArrowLeft size={19} />
        </button>
